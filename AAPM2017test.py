...
path_lbl = '/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/2017aapmmasks/'
path_img = '/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/2017aapmimages/'
...
codes= array(['SpinalCord', 'Lung_R', 'Lung_L', 'Heart', 'Esophagus','Void'])
...
#    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/testing3d/Patient_'+str(i)+'_GT.nii.gz')
#path1='/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPMdataset/testdata/'
...
#        #img=img[1,:,:]
#        x = np.array(img)
#        t = pil2tensor(x, dtype=np.float32) # converts to numpy tensor
#        im = Image(t) # Convert to fastAi Image - this class has "apply_tfms"
#        im_g=learn.predict(im)
#        na=np.array(im_g[1][0])
#        X_train[n] = na
#        ff=np.swapaxes(X_train,0,2)
#    i=i+24
#    ex="/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPMdataset/testaapmnib//Patient_"+str(i)+".nii.gz"    
#    img = nib.load(ex)
#    im1=np.array(img.get_affine())
#    new_image = nib.Nifti1Image(np.asarray(ff,dtype="uint8" ), affine=im1)
#    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPMdataset/3d_images/Patient_'+str(i)+'_GT.nii.gz')
#  
import matplotlib
from matplotlib import pyplot as plt
...
#    new_image = nib.Nifti1Image(np.asarray(na,dtype="uint8" ), affine=im1)
#    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/testing3d/Patient_'+str(i)+'_GT.nii.gz')
path1='/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/testimagesfolder1/'
oslist=os.listdir(path1)
import natsort
import matplotlib.pyplot as plt
im_height=512
...
for i, volume in enumerate(oslist):
    ...
    for n, id_ in tqdm(enumerate(files), total=len(files)):
        print(n)
        print(id_)
        img=open_image(os.path.join(cur_path,id_))
        print(img.shape)
        #img=np.swapaxes(img,0,2)
        #img1=img.transpose(1, 2, 0)
        #img=img[1,:,:]
        ...
        ff=np.swapaxes(X_train,0,2)
    i=i
    ex="/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/testinputniiimages/Patient_"+str(i)+".nii.gz"    
    img = nib.load(ex)
    im1=np.array(img.get_affine())
    new_image = nib.Nifti1Image(np.asarray(ff,dtype="uint8" ), affine=im1)
    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/3doutput/Patient_'+str(i)+'_Predict.nii.gz')
...
for i, volume in enumerate(oslist):
    ...
    im1=np.array(img.get_affine())
    new_image = nib.Nifti1Image(np.asarray(ff,dtype="uint8" ), affine=im1)
    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/3doutput1/Patient_'+str(i)+'_Predict.nii.gz')
...
for i, volume in enumerate(oslist):
    ...
    im1=np.array(img.get_affine())
    new_image = nib.Nifti1Image(np.asarray(ff,dtype="uint8" ), affine=im1)
    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/3doutput11/Patient_'+str(i)+'_Predict.nii.gz')
...
for i, volume in enumerate(oslist):
    ...
    im1=np.array(img.get_affine())
    new_image = nib.Nifti1Image(np.asarray(ff,dtype="uint8" ), affine=im1)
    nib.save(new_image,'/raid/Home/Users/aqayyum/TestMulticlass/fastaicodes/AAPM2017/3doutput111/Patient_'+str(i)+'_predict.nii.gz')
